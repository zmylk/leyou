...
/**
 * @功能概要：调用DEMO @公司名称： ShenZhen Montnets Technology CO.,LTD.
 */
public class Test {
	// 日期格式定义
...
//		ConfigManager.removeAllIpInfo();*/
	...
	public static void singleSend(String userid, String pwd, boolean isEncryptPwd) {
		try {
			...
			// 设置手机号码 此处只能设置一个手机号码
			message.setMobile("18845120147");
			// 设置内容
			message.setContent("恭喜你，支付成功！");
			// 设置扩展号
			message.setExno("11");
			// 用户自定义流水编号
	...
	public static void batchSend(String userid, String pwd, boolean isEncryptPwd) {
		try {
			...
			// 设置手机号码
			message.setMobile("15814129738,18320461421");
			// 设置内容
			message.setContent("尊敬的客户，您的邮件已到达，感谢您的支持，谢谢！");
			// 设置扩展号
			message.setExno("11");
			// 用户自定义流水编号
			message.setCustid("20160929194950100001");
